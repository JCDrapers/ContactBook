...
def row = sql.firstRow("select max(id) as max from Contacts")
def Number_Of_Contacts = row.max
                    ...
                    (0..numPanels-1).each { num ->
                        ...
                        def pane = panel(alignmentX: 0f, id: panelID, background: java.awt.Color.LIGHT_GRAY, layout: new GridLayout(1, 2)) {
                            ...
                            remove_button = button(id: "buttonpanel$num", text: "Remove", actionPerformed: {
                                frame.setEnabled(false)
                                frame3 = swing.frame(title: "Remove $Contact?", pack: true, visible: true, defaultCloseOperation: WC.DO_NOTHING_ON_CLOSE) {
                                    panel(id: 'thirdPanel') {
                                        vbox {
                                            sql.eachRow("select * from Contacts WHERE ID = $num+1") {
